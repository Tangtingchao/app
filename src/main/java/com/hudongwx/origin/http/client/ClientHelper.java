...
public class ClientHelper {
        ...
        return new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                request = request.newBuilder()
                        .header("appv",OSUtil.getVersionCode()+"")
                        .build();

                boolean isNet = OSUtil.hasInternet();
                if (!isNet) {
                ...
                Response response = chain.proceed(request);
                if (isNet) {//有网络情况下，根据请求接口的设置
                    String cacheControl = request.cacheControl().toString();
                    if (cacheControl != null && cacheControl.trim().length() !=0) {//服务端设置了缓存策略，则使用服务端缓存策略
    ...
    public static Interceptor getAuthorization() {
        Interceptor headerInterceptor = new Interceptor() {
            @Override
            public Response intercept(Chain chain) throws IOException {
                Request originalRequest = chain.request();//获取请求对象,添加token
                Request.Builder builder = originalRequest.newBuilder();
                builder.header("token", "");//添加令牌
                Request.Builder requestBuilder = builder.method(originalRequest.method(), originalRequest.body());
                Request request = requestBuilder.build();
                return chain.proceed(request);
            }
